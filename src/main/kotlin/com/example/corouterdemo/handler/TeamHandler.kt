...
class TeamHandler(
    ...
    suspend fun create(request: ServerRequest): ServerResponse {
        ...
        return ServerResponse.status(HttpStatus.CREATED).bodyValueAndAwait(team.toResponse())
    }

    suspend fun findById(request: ServerRequest): ServerResponse {
        val id = request.pathVariable("id").toLong()
        val team = teamService.findById(id)
        return ServerResponse.ok().bodyValueAndAwait(team.toResponse())
    }

    suspend fun findAll(request: ServerRequest): ServerResponse =
        ServerResponse.ok().bodyAndAwait(
            teamService.findAll().map {
                it.toResponse()
            },
        )
    ...
    suspend fun findByEnterprise(request: ServerRequest): ServerResponse {
        ...
        return ServerResponse.ok().bodyAndAwait(teamService.findByEnterprise(enterpriseId).map { it.toResponse() })
    }
