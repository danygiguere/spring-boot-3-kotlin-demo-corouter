...
- Never pick a bundle yourself — with no args, print the bundle list and stop. Full-scope sweeps must name a bundle + scope.
...
## Notes
- Complements (doesn't replace) the built-in `/code-review` + `/security-review` (generic). `audit` is for running the project's domain audits at **bundle / layer / repo** scope.
