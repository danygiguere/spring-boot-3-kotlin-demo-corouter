---
name: code-review-pr
description: Code-review a pull request and post the result as a PR comment — 5 parallel reviewers (conventions, bugs, git history, prior-PR comments, code comments) filtered by a confidence score. Renamed copy of Anthropic's built-in code-review command so it doesn't shadow /code-review. Use to leave a review comment on a GitHub PR.
tags: [meta]
---
...
> Changes from the original: renamed to `code-review-pr` (to avoid shadowing the
> built-in `/code-review`); frontmatter converted to this repo's skill format; the
> CLAUDE.md references below mean **`AGENTS.md`** in this project.
